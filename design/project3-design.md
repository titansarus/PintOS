...
```c
/* inode.c */
#define DIRECT_BLK_CNT 124
#define IND_BLK_CNT 128
#define DBL_IND_BLK_CNT 128

struct inode_disk {
  ...
  block_sector_t db[DIRECT_BLK_CNT];    // size: (124)*4
  indirect_block_t *ib;                 // size: 4
  double_indirect_block_t *dib;         // size: 4
  ...
...
typedef struct indirect_block indirect_block_t;

struct double_indirect_block {
  indirect_block_t indirect_blocks[DBL_IND_BLK_CNT];
};

typedef struct double_indirect_block double_indirect_block_t;
...
`inode`
تصادفی می‌باشد، عدالت بین ریسه‌های نویسنده و خواننده رعایت می‌شود.
